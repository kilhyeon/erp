...
//		pContent.setLayout(new GridLayout(1, 0, 0, 0));
	...
	private JPopupMenu createPopupMenu() {
		...
		JMenuItem empListByDepartmentItem = new JMenuItem("해당 부서 사원 보기");
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (e.getActionCommand().equals("수정")) {
					...
					pContent.setDept(updateDept);
					btnAdd.setText("수정");
				}
				
				if (e.getActionCommand().contentEquals("해당 부서 사원 보기")) {
					/*
					...
					if (list == null) {
						JOptionPane.showMessageDialog(null, "해당 사원이 없음", "해당 부서 사원", JOptionPane.INFORMATION_MESSAGE);
						return;
					}
					...
					JOptionPane.showMessageDialog(null, strList, "해당 부서 사원", JOptionPane.INFORMATION_MESSAGE);
				}
...
//			pContent.clearTf();
	...
	private void actionPerformedBtnUpdate(ActionEvent e) {
		...
		Department updateDepartment = pContent.getDept();
		service.modifyDept(updateDepartment);
		pList.loadData();
		pContent.clearTf();
		btnAdd.setText("추가");
		JOptionPane.showMessageDialog(null, updateDepartment.getDeptName() + "정보가 수정되었습니다.");
	}
	
	protected void actionPerformedBtnAdd(ActionEvent e) {
		Department department = pContent.getDept();
